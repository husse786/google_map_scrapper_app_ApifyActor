...
Dieses Projekt ist eine Anwendung mit einer grafischen Benutzeroberfläche (GUI), um den Prozess des Extrahierens und Bereinigens von Unternehmensdaten aus Google Maps zu automatisieren. Die Anwendung nutzt die Apify-Plattform im Hintergrund, um gezielte und genaue Suchanfragen parallel auszuführen.
...
Ja, absolut. Ihre alte `README.md`-Datei war nicht mehr aktuell und spiegelte die vielen Verbesserungen, die wir vorgenommen haben, nicht wider.

Hier ist die komplett aktualisierte und professionell formatierte `README.md`-Datei. Sie enthält jetzt alle korrekten Informationen über die finale Architektur, die parallele Verarbeitung, den zweistufigen Workflow und die intelligente Datenbereinigung.

Sie können diesen gesamten Block kopieren und direkt in Ihre `README.md`-Datei auf GitHub einfügen.
